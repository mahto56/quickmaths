sy = ['dots', 'tan', ')', '(', '+', '-', 'sqrt', '1', '0', '3', '2', '4', '6', 'mul', 'pi', '=', 'sin', 'pm', 'A','frac', 'cos', 'delta', 'a', 'c', 'b', 'bar', 'd', 'f', 'i', 'h', 'k', 'm', 'o', 'n', 'p', 's', 't', 'y', 'x', 'div']
...
def updateEqual(symbol,s1,symbol_list, im, i):
    ...
    new_symbol = (im[new_y:new_yh,new_x:new_xw].copy(), "=", new_x, new_y, new_xw, new_yh)
...
def updateDivision(symbol,s1,s2,symbol_list, im, i):
    ...
    new_symbol = (im[new_y:new_yh,new_x:new_xw].copy(), "div", new_x, new_y, new_xw, new_yh)
    symbol_list[i] = new_symbol
    symbol_list.pop(i+2)
    symbol_list.pop(i+1)
...
def updateDots(symbol,s1,s2,symbol_list, im, i):
    ...
    new_xw = max(symbol[4], s1[4], s2[4])
    new_yh = max(symbol[5], s1[5], s2[5])
    new_symbol = (im[new_y:new_yh,new_x:new_xw].copy(), "dots", new_x, new_y, new_xw, new_yh)
    symbol_list[i] = new_symbol
    symbol_list.pop(i+2)
    symbol_list.pop(i+1)
...
def updateI(symbol,s1,symbol_list, im, i):
    ...
    new_xw = max(symbol[4], s1[4])
    new_yh = max(symbol[5], s1[5])
    new_symbol = (im[new_y:new_yh,new_x:new_xw].copy(), "i", new_x, new_y, new_xw, new_yh)
    symbol_list[i] = new_symbol
    symbol_list.pop(i+1)
...
def updatePM(symbol,s1,symbol_list, im, i):
    ...
    new_xw = max(symbol[4], s1[4])
    new_yh = max(symbol[5], s1[5])
    new_symbol = (im[new_y:new_yh,new_x:new_xw].copy(), "pm", new_x, new_y, new_xw, new_yh)
    symbol_list[i] = new_symbol
    symbol_list.pop(i+1)
...
def update(im,symbol_list):
	slen = len(symbol_list)
	for i in range(slen):
